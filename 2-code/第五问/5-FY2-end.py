...
delta_t1 = 20  # 第一个烟雾弹投放时间范围扩大
delta_t2 = 30  # 第二个烟雾弹投放时间范围
delta_t3 = 20  # 第三个烟雾弹投放时间范围
min_theta = 25
# 配置日志
...
class EnhancedAdaptiveDE_LBFGS_Optimizer:
    def __init__(self, pop_size: int = 120, f_min: float = 0.5, f_max: float = 1.0,
                 ...
                 max_restarts: int = 30, diversity_threshold: float = 0.15,  # 提高多样性阈值
    ...
    def fitness(self, x: np.ndarray) -> float:
        ...
        theta_int = initjude(1,1)
        

        
        vi = np.array([0,105 + x[0] * (70) - 35])
       
        theta =  np.array([0,theta_int +((x[1] * 2*min_theta-min_theta) * np.pi / 180) ])
        # 调整时间参数，确保三个烟雾弹有足够的时间间隔和作用窗口
        
        
        # 第一个烟雾弹
        tdrop_1 = x[2] * delta_t1  # 投放时间:0-15
        texpl_1 = tdrop_1 + x[3] * 20  # 爆炸时间:投放后0-20秒
        
        # 第二个烟雾弹（确保在第一个之后投放）
        tdrop_2 = tdrop_1 + 1 + x[4] * delta_t2  # 投放时间:第一个+1秒后，再0-30秒
        texpl_2 = tdrop_2 + x[5] * 20  # 爆炸时间:投放后0-20秒
        
        # 第三个烟雾弹（确保在第二个之后投放）
        tdrop_3 = tdrop_2 + 1 + x[6] * delta_t3  # 投放时间:第二个+1秒后，再0-30秒
        texpl_3 = tdrop_3 + x[7] * 20  # 爆炸时间:投放后0-20秒
        
        # 整理投放和爆炸时间数组（3个烟雾弹）
        tdrop = np.array([[100000, 100000, 100000],
                          [tdrop_1, tdrop_2, tdrop_3]])
        texpl = np.array([[100000, 100000, 100000],
                          [texpl_1, texpl_2, texpl_3]])
...
if __name__ == "__main__":
    ...
    def denormalize_params(x):
        ...
        params = {
            ...
            "角度 (度)": np.rad2deg(theta_int_11 + (x[1] * 2*min_theta - min_theta) * np.pi / 180),
            "第一个烟雾弹投放时间 (s)": x[2] * delta_t1,
            "第一个烟雾弹爆炸时间 (s)": x[2] * delta_t1 + x[3] * 20,
            "第二个烟雾弹投放时间 (s)": x[2] * delta_t1 + 1 + x[4] * delta_t2,
            "第二个烟雾弹爆炸时间 (s)": (x[2] * delta_t1 + 1 + x[4] * delta_t2) + x[5] * 20,
            "第三个烟雾弹投放时间 (s)": (x[2] * delta_t1 + 1 + x[4] * delta_t2) + 1 + x[6] * delta_t3,
            "第三个烟雾弹爆炸时间 (s)": ((x[2] * delta_t1 + 1 + x[4] * delta_t2) + 1 + x[6] * delta_t3) + x[7] * 20
        }
        return params
