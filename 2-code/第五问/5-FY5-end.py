...
class EnhancedAdaptiveDE_LBFGS_Optimizer:
    def __init__(self, pop_size: int = 120, f_min: float = 0.5, f_max: float = 1.0,
                 cr_min: float = 0.2, cr_max: float = 1.0, max_generations: int = 400,
                 restart_threshold: float = 5e-6, max_stagnation: int = 15,
                 max_restarts: int = 50, diversity_threshold: float = 0.15,
    ...
    def fitness(self, x: np.ndarray) -> float:
        """适应度计算：烟雾弹阻断时间"""
        # 基础参数计算
        theta_int = self._initjude(4, 0)  # 统一使用内部方法计算角度
        
        # 速度和角度参数
        vi = np.array([0, 0, 0, 0, 105 + x[0] * 70 - 35])  # v = 70~140
        theta = np.array([0, 0, 0, 0, theta_int + (x[1] * 2*min_theta - min_theta) * np.pi / 180])  # 角度偏移±15度
        ...
        for t in np.linspace(0, 67, pace):
            Mj, smokes_location = cover(t, 0)  # 追逐M1
    ...
    def denormalize_params(self, x: np.ndarray) -> dict:
        ...
        theta_int = self._initjude(4, 0)  # 与fitness中使用相同的基准角度
