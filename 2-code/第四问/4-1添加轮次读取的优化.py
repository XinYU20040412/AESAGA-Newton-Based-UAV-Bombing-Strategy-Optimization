...
class AESAGAWithNewton:
    ...
    def __init__(self,
        ...
        self.restart_threshold = restart_threshold  # 提高重启阈值
    ...
    def load_best_parameters(self, file_path: str = 'aesaga_newton_best_result.txt') -> np.ndarray:
        ...
        try:
            ...
            # 解析3枚导弹的参数（每枚4个参数）
            for i in range(3):
                # 找到第i+1枚导弹的参数行（格式："导弹 i+1:" 开始）
                missile_line = f"导弹 {i+1}:"
                start_idx = next(idx for idx, line in enumerate(lines) if missile_line in line)
                
                # 解析速度（转换为比例：(vi-70)/70）
                vi_line = lines[start_idx + 1].strip()
                vi = float(vi_line.split(':')[1].strip().split()[0])
                vi_ratio = (vi - 70) / 70  # 转换为[0,1]范围
                
                # 解析角度（转换为比例：(角度 - 基础角度 + 15°)/30°）
                theta_line = lines[start_idx + 2].strip()
                theta_deg = float(theta_line.split(':')[1].strip().split()[0])
                theta_base_deg = self.theta_base[i] * 180 / np.pi  # 基础角度转换为度
                theta_ratio = (theta_deg - theta_base_deg + 15) / 30  # 转换为[0,1]范围
                
                # 解析释放时间（转换为比例：tdrop/10）
                tdrop_line = lines[start_idx + 3].strip()
                tdrop = float(tdrop_line.split(':')[1].strip().split()[0])
                tdrop_ratio = tdrop / 10  # 转换为[0,1]范围
                
                # 解析起爆时间差（转换为比例：texpl_diff/10）
                texpl_diff_line = lines[start_idx + 4].strip()
                texpl_diff = float(texpl_diff_line.split(':')[1].strip().split()[0])
                texpl_diff_ratio = texpl_diff / 10  # 转换为[0,1]范围
                
                # 收集当前导弹的4个比例参数
                params.extend([vi_ratio, theta_ratio, tdrop_ratio, texpl_diff_ratio])
            
            # 裁剪到[0,1]范围（避免解析误差）
            return np.clip(np.array(params), 0, 1)
        
        except (FileNotFoundError, StopIteration, IndexError, ValueError) as e:
    ...
    def fitness(self, x: np.ndarray) -> float:
        ...
        for i in range(3):
            ...
            # 角度: 基础角度 ±15度
            angle_adjustment = (x[i, 1] * min_theta*2 - min_theta) * np.pi / 180  # 转换为弧度
            theta.append(self.theta_base[i] + angle_adjustment)

            # 释放时间: 0-10秒
            tdrop.append([x[i, 2] * dleta_tdrop])

            # 起爆时间差: 0-10秒，起爆时间 = 释放时间 + 时间差
            texpl.append([x[i, 3] * dleta_texpl_diff + x[i, 2] * dleta_tdrop])
    ...
    def initialize_population(self) -> np.ndarray:
        ...
        if best_prev is not None:
            ...
            # 剩余个体用拉丁超立方抽样生成（保证多样性）
            for i in range(num_based, self.pop_size):
                for j in range(n_dim):
                    pop[i, j] = np.random.uniform(0, 1)
        else:
            # 无上次参数，完全用拉丁超立方抽样初始化
    ...
    def restart_population(self, best_individual: np.ndarray) -> np.ndarray:
        ...
        for i in range(1, int(self.pop_size * 0.3)):
            new_pop[i] = best_individual + np.random.normal(0, 0.1, len(best_individual))  # 增加扰动幅度
        # 生成全新个体
    ...
    def optimize(self, generations: int = 150) -> Tuple[np.ndarray, float, List[float], List[float]]:
        ...
        for generation in range(generations):
            ...
            # 偶尔引入随机个体增加多样性
            if np.random.random() < 0.08:  # 增加随机个体引入概率
                random_idx = np.random.randint(0, self.pop_size)
                new_population[random_idx] = np.random.random(12)  # 12维参数
...
if __name__ == "__main__":
    ...
    optimizer = AESAGAWithNewton(
        ...
        alpha=0.97,
        beta=1.0,
        max_stagnation=6,  # 减少停滞容忍代数
        newton_max_iter=30,
        newton_tol=1e-5,
        restart_threshold=0.01,  # 提高重启阈值
        diversity_threshold=0.1
    )

    best_individual, best_fitness, best_history, avg_history = optimizer.optimize(generations=200)  # 增加迭代次数
    ...
    for i in range(3):
        ...
        theta_deg = (best_individual[i, 1] * 2*min_theta - min_theta) + optimizer.theta_base[i] * 180 / np.pi
        tdrop = best_individual[i, 2] * dleta_tdrop
        texpl_diff = best_individual[i, 3] * dleta_texpl_diff
