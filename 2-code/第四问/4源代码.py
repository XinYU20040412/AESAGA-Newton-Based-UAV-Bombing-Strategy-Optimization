...
class AESAGAWithNewton:
    ...
    def fitness(self, x: np.ndarray) -> float:
        ...
        for i in range(3):
            ...
            # 角度: 基础角度 ±15度
            angle_adjustment = (x[i, 1] * 50 - 25) * np.pi / 180  # 转换为弧度
            theta.append(self.theta_base[i] + angle_adjustment)

            # 释放时间: 0-10秒
            tdrop.append([x[i, 2] * 15])

            # 起爆时间差: 0-10秒，起爆时间 = 释放时间 + 时间差
            texpl.append([x[i, 3] * 15 + x[i, 2] * 15])
...
if __name__ == "__main__":
    ...
    for i in range(3):
        ...
        theta_deg = (best_individual[i, 1] * 30 - 15) + optimizer.theta_base[i] * 180 / np.pi
        tdrop = best_individual[i, 2] * 10
        texpl_diff = best_individual[i, 3] * 10
