...
#参数组成为x=[速度标准化参数，方向标准化参数，一无人机一投放参数，一无人机二投放时间差参数，，，一无人机一爆炸参数]
# 设置日志
...
class AESAGAWithNewton:
    ...
    def __init__(self,
                 ...
                 beta: float = 1.0,
                 max_stagnation: int = 2,  # 增加停滞容忍度
                 newton_max_iter: int = 30,  # 增加牛顿法迭代次数
                 newton_tol: float = 1e-3,  # 提高牛顿法精度
    ...
    def initialize_population(self) -> np.ndarray:
        ...
        n_dim = 10+2*np.sum(self.smoke_num)  # 3枚导弹 × 4个参数
    ...
    def optimize(self, generations: int = 150) -> Tuple[np.ndarray, float, List[float], List[float]]:
        ...
        for generation in range(generations):
            ...
            if stagnation_count >= self.max_stagnation and self.restart_count < self.max_restarts:
                ...
                T = self.T0 * (0.97 ** self.restart_count)  # 重启后温度降低幅度减小
            ...
            if len(new_population) > self.pop_size:
                ...
                while len(new_population) < self.pop_size:
                    new_population.append(np.random.random(10+2*np.sum(self.smoke_num)))

            # 偶尔引入随机个体增加多样性
            if np.random.random() < 0.08:  # 增加随机个体引入概率
                random_idx = np.random.randint(0, self.pop_size)
                new_population[random_idx] = np.random.random(10+2*np.sum(self.smoke_num))
...
if __name__ == "__main__":
    ...
    # cover_plan,smoke_num=pro5_deter_fun()



    optimizer = AESAGAWithNewton([0,1,2,1,0],[3,3,1,2,2],
        pop_size=60,  # 增加种群规模
        elite_size=0.15,
        Pc1=0.95,
        Pc2=0.4,
    ...
    # 获取基础角度（注意：problem3中只有一枚导弹，所以init_theta([1])返回的数组只有一个元素）
    base_theta = init_theta([1])[0] * 180 / np.pi  # 转换为角度

    # 计算实际参数
    vi = 110 + best_individual[0] * 60 - 30  # 速度范围:70-140
    theta_adjust = (best_individual[1] * 30 - 15)  # 角度调整范围:-15°至15°
    theta_actual = base_theta + theta_adjust  # 实际角度
    ...
    # 第一个烟雾弹
    tdrop1 = best_individual[2] * delta_t1
    texpl1 = tdrop1 + best_individual[3] * 15

    # 第二个烟雾弹
    tdrop2 = tdrop1 + 1 + best_individual[4] * delta_t2
    texpl2 = tdrop2 + best_individual[5] * 15

    # 第三个烟雾弹
    tdrop3 = tdrop2 + 1 + best_individual[6] * delta_t3
    texpl3 = tdrop3 + best_individual[7] * 15

    print("\n最佳参数:")
    print(f"速度: {vi:.2f} m/s")
    print(f"基础角度: {base_theta:.2f}°")
    print(f"角度调整: {theta_adjust:.2f}°")
    print(f"实际角度: {theta_actual:.2f}°")
    print(f"第一个烟雾弹投放时间: {tdrop1:.2f} s, 爆炸时间: {texpl1:.2f} s")
    print(f"第二个烟雾弹投放时间: {tdrop2:.2f} s, 爆炸时间: {texpl2:.2f} s")
    print(f"第三个烟雾弹投放时间: {tdrop3:.2f} s, 爆炸时间: {texpl3:.2f} s")
    ...
    with open('aesaga_newton_best_result.txt', 'w', encoding='utf-8') as f:
        ...
        f.write(f"速度: {vi:.2f} m/s\n")
        f.write(f"基础角度: {base_theta:.2f}°\n")
        f.write(f"角度调整: {theta_adjust:.2f}°\n")
        f.write(f"实际角度: {theta_actual:.2f}°\n")
        f.write(f"第一个烟雾弹投放时间: {tdrop1:.2f} s, 爆炸时间: {texpl1:.2f} s\n")
        f.write(f"第二个烟雾弹投放时间: {tdrop2:.2f} s, 爆炸时间: {texpl2:.2f} s\n")
        f.write(f"第三个烟雾弹投放时间: {tdrop3:.2f} s, 爆炸时间: {texpl3:.2f} s\n")
